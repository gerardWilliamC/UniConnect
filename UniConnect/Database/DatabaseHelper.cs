...
namespace UniConnect.Database
{
    public class DatabaseHelper
    {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            grades.Add(new Grade
                            {
                                ...
                                GradeValue = reader["grade"] == DBNull.Value ? (decimal?)null : Convert.ToDecimal(reader["grade"]),
        ...
        public (int totalUnits, int passedCount, int failedCount) GetGradesSummary(string studentId, string semester)
        {
            string sql = @"
                SELECT
        ...
        public (decimal? gwa, int totalUnits, int passedCount, int failedCount) GetOverallGradesSummary(string studentId)
        {
            string sql = @"
                SELECT
            ...
            using (SqlConnection conn = GetConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            decimal? gwa = reader["gwa"] == DBNull.Value ? (decimal?)null : Convert.ToDecimal(reader["gwa"]);
                            int totalUnits = reader["total_units"] == DBNull.Value ? 0 : Convert.ToInt32(reader["total_units"]);
                            int passedCount = reader["passed_count"] == DBNull.Value ? 0 : Convert.ToInt32(reader["passed_count"]);
                            int failedCount = reader["failed_count"] == DBNull.Value ? 0 : Convert.ToInt32(reader["failed_count"]);
        ...
        {
            string sql = @"SELECT DISTINCT semester FROM grades WHERE student_id = @StudentId";
            ...
            using (SqlConnection conn = GetConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read()) semesters.Add(reader["semester"].ToString());
                    }
                }
            }
            ...
            semesters.Sort((a, b) =>
            {
                var matchA = System.Text.RegularExpressions.Regex.Match(a, @"\d{4}");
                var matchB = System.Text.RegularExpressions.Regex.Match(b, @"\d{4}");
                int yearA = matchA.Success ? int.Parse(matchA.Value) : 0;
                int yearB = matchB.Success ? int.Parse(matchB.Value) : 0;
        ...
        public List<Announcement> GetAnnouncements(string targetAudience = null, int limit = 50, string studentId = null)
        {
            string sql = @"
                SELECT TOP (@Limit)
                ...
                WHERE a.is_archived = 0
                  AND (@Audience IS NULL OR a.target_audience = 'All' OR a.target_audience = @Audience)
            ...
            using (SqlConnection conn = GetConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Audience", (object)targetAudience ?? DBNull.Value);
        ...
        public void MarkAnnouncementAsRead(string studentId, int announcementId)
        {
            string sql = @"
                IF NOT EXISTS (
                    ...
                    WHERE student_id = @StudentId AND announcement_id = @AnnouncementId
                )
        ...
        public (int totalStudents, int totalCourses, int pendingGrades, int announcements) GetAdminDashboardCounts()
        {
            string sql = @"
                SELECT
        ...
        public List<(string studentName, string studentId, string subjectCode, string subjectName, decimal? grade, string status, string editedBy, DateTime updatedAt)> GetRecentGradeEntries(int limit = 10)
        {
            string sql = @"
                SELECT TOP (@Limit)
                       st.full_name AS student_name, g.student_id, g.subject_code,
                       s.subject_name, g.grade, g.status,
        ...
        public List<AuditLog> GetRecentAuditLogs(int limit = 8)
        {
            string sql = @"
                SELECT TOP (@Limit)
                       al.log_id, al.action_type, al.table_affected, al.performed_by,
        ...
        public Student FindStudent(string query)
        {
            string sql = @"
                ...
                FROM students
                WHERE student_id = @Query OR full_name LIKE @Like
                ORDER BY student_id";
        ...
        public void UpdateGradeWithAudit(string studentId, string subjectCode, decimal? newGrade, string newStatus, string semester, string adminId, string detailsForLog)
        {
            using (SqlConnection conn = GetConnection())
            {
                conn.Open();
                using (SqlTransaction tx = conn.BeginTransaction())
                {
                    try
                    {
                        string updateSql = @"
                            UPDATE grades
                            SET grade = @Grade, status = @Status, updated_by = @AdminId, updated_at = GETDATE()
                            WHERE student_id = @StudentId AND subject_code = @SubjectCode AND semester = @Semester";

                        using (SqlCommand cmd = new SqlCommand(updateSql, conn, tx))
                        {
                            cmd.Parameters.AddWithValue("@Grade", (object)newGrade ?? DBNull.Value);
        ...
        public int PostAnnouncementWithAudit(string title, string content, string targetAudience, string adminId)
        {
            using (SqlConnection conn = GetConnection())
            {
                conn.Open();
        ...
        public void ArchiveAnnouncementWithAudit(int announcementId, string adminId, string title)
        {
            using (SqlConnection conn = GetConnection())
            {
                ...
                using (SqlTransaction tx = conn.BeginTransaction())
                {
                    try
                    {
                        string updateSql = @"UPDATE announcements SET is_archived = 1 WHERE announcement_id = @Id";
                        using (SqlCommand cmd = new SqlCommand(updateSql, conn, tx))
                        {
        ...
        // ==========================================
        // NEW METHOD FOR PENDING QUEUE SIDEBAR
        // ==========================================
        public List<(string studentId, string fullName, int pendingCount)> GetPendingStudents()
        {
            string sql = @"
                SELECT st.student_id, st.full_name, COUNT(g.grade_id) as pending_count
                FROM students st
                INNER JOIN grades g ON st.student_id = g.student_id
                WHERE g.status = 'Pending' OR g.grade IS NULL
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        list.Add((
                            ...
                            Convert.ToInt32(reader["pending_count"])
                        ));
                    }
                }
            }
